defmodule LangChain.MessageProcessors.ChangesetProcessor do
  @moduledoc """
  A built-in Message processor that processes a received Message into the
  provided `Ecto.Changeset`.

  `EctoChangesetProcessor`? `EctoProcessor`? `ChangesetProcessor`?


  Don't want it to try an update this way. The ID issue. I want updates to go
  through my context.

      EctoProcessor.new!(%{repo: Repo})

      EctoProcessor.new!(%{apply: true/false})

  Return a changeset with or without errors or apply the changeset and return an
  {:ok, Struct} | {:error, changeset}



  TODO: PROBLEM. The JSONSchema still needs to be defined separately from the Ecto struct. They would need to be kept in sync but separate. Not ideal. That's what Instructor handles rather nicely.

  The other approach to show is using a function to interact with a Phoenix context (business layer) and also through a real database-backed Ecto Schema.
