defmodule Thunderline.PAC.AgentTickSteps do
      ...
      "explore" ->
        ...
        case validate_movement(agent, explore_params) do
          ...
          {:error, movement_error} ->
            ...
            case Thunderline.AI.Tools.Explore.explore(
                   Map.put(explore_params, :intensity, "cautious"),
                   %{agent: agent}
                 ) do
              {:ok, explore_result} ->
  ...
  def form_memory(
        %{context: context, decision: decision, action_result: action_result, agent: agent},
        _reactor_context
      ) do
  ...
  def update_agent_state(
        %{agent: agent, action_result: action_result, memory_formed: memory},
        _context
      ) do
    ...
    result_data =
      case action_result do
        ...
        {:error, _} = error ->
          %{result_message: "Action failed: #{inspect(error)}", energy_change: 0}

        data when is_map(data) ->
          data

        _ ->
          %{result_message: "Unknown result", energy_change: 0}
      end
    ...
    new_activity =
      case Map.get(result_data, :result_message, "") do
    ...
    case Thunderline.PAC.Agent.update(agent, %{stats: new_stats, state: new_state},
  ...
  defp zone_accessible?(zone) do
    ...
    # Zone must be at least 50% accessible
    accessibility > 0.5
  end
end
