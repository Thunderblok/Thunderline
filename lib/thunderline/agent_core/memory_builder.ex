defmodule Thunderline.AgentCore.MemoryBuilder do
  ...
  # Contains action outcome, success, execution_result, etc.
  @type experience_map :: map()
  # %{pac_id: String.t(), pac_name: String.t(), stats: map(), traits: list(String.t()), personality: map()}
  @type pac_config_map :: map()
  # %{zone_context: map(), ...}
  @type tick_context_map :: map()
  # Agent's state before the action
  @type previous_state_map :: map()
  # Contains memories_formed, memory_types, etc.
  @type memory_report_map :: map()
  ...
  def store_results(experience, pac_config, tick_context, prev_state) do
    ...
    with {:ok, memory_candidates} <- extract_memory_candidates(experience, tick_context),
      ...
      report = %{
        ...
        memories_formed_count: length(stored_memories),
        memory_types: extract_memory_types(stored_memories),
        # Changed key for clarity
        significant_events_descriptions: extract_significant_events(processed_memories),
        # Changed key
        overall_emotional_impact: calculate_emotional_impact(processed_memories),
      ...
      {:error, reason} ->
        Logger.error(
          "MemoryBuilder: Memory formation failed for PAC #{pac_name}: #{inspect(reason)}"
        )
      ...
      unexpected_error ->
        Logger.error(
          "MemoryBuilder: Unexpected error for PAC #{pac_name}: #{inspect(unexpected_error)}"
        )
  ...
  defp extract_memory_candidates(experience, context) do
    ...
    # Default to false if :success is nil
    candidates =
      if Map.get(experience, :success, false) do
    ...
    candidates =
      if action_result[:type] == :social_interaction do
        # Pass full experience
    ...
    candidates =
      if learning = action_result[:learning] do
    ...
    candidates =
      if creation = action_result[:creation] do
  ...
  defp create_action_memory(experience, context) do
    ...
    %{
      ...
      description:
        action_result[:description] || "Performed #{Map.get(experience, :action, "action")}",
      energy_cost: action_result[:energy_cost] || 0,
      mood_effect: action_result[:mood_effect],
      # Changed key from :context
  ...
  defp create_failure_memory(experience, context) do
    %{
      ...
      description:
        "Failed to #{Map.get(experience, :action, "action")}: #{inspect(Map.get(experience, :error, :unknown_error))}",
      context_summary: extract_context_summary(context),
      importance: 0.7,
  ...
  defp create_discovery_memory(discovery, context) do
    %{
      type: :discovery,
      # Changed key from :discovery to :content for consistency
      content: discovery,
      # Use inspect for safety
      description: "Discovered: #{inspect(discovery)}",
      context_summary: extract_context_summary(context),
      importance: 0.8,
  ...
  # Changed param name
  defp create_learning_memory(learning_content, context) do
    %{
      type: :learning,
      # Changed key from :learning
      content: learning_content,
      # Use inspect
      description: "Learned: #{inspect(learning_content)}",
      context_summary: extract_context_summary(context),
      importance: 0.8,
  ...
  # Changed param name
  defp create_creative_memory(creation_content, context) do
    %{
      type: :creative,
      # Changed key from :creation
      content: creation_content,
      # Use inspect
      description: "Created: #{inspect(creation_content)}",
      context_summary: extract_context_summary(context),
      importance: 0.9,
  ...
  defp process_memories(candidates, pac_config, prev_state) do
    processed =
      candidates
      # Renamed for clarity
      |> Enum.map(&enrich_memory(&1, pac_config, prev_state))
      |> Enum.filter(&filter_significant_memories/1)
      |> Enum.map(&add_temporal_context/1)
      # This adds :embedding_text field
  ...
  defp enrich_memory(memory_candidate, pac_config, prev_state) do
    ...
    base_memory_data = %{
      ...
      # Default importance
      importance: Map.get(memory_candidate, :importance, 0.5),
      tags: Map.get(memory_candidate, :tags, []),
      emotional_valence: Map.get(memory_candidate, :emotional_valence, 0.0),
      # type from candidate
      type: Map.get(memory_candidate, :type, :general_event),
      # For discovery, learning, creative
      content: Map.get(memory_candidate, :content)
    }
    ...
    # Using base data for now without explicit AI call here
    final_memory_data = base_memory_data
  ...
  defp generate_embedding_text(memory) do
    ...
    text_parts = [
      ...
      # Ensure type is string/atom
      "Type: #{Atom.to_string(memory.type)}",
  ...
  defp adjust_importance_for_personality(memory, personality_map) when is_map(personality_map) do
    ...
    multiplier =
      case {memory.type, personality_map.decision_tendency} do
        {:learning, "analytical"} -> 1.3
        # Assuming "innovative" is a tendency
    ...
    updated_importance = min(1.0, (memory.importance || 0.5) * multiplier)
    Map.put(memory, :importance, updated_importance)
  end

  # No personality map
  defp adjust_importance_for_personality(memory, _), do: memory
  ...
  defp store_memories(memories_to_store, pac_id) when is_list(memories_to_store) do
    ...
    if !Enum.empty?(failures) do
      Logger.error(
        "MemoryBuilder: Failed to store #{length(failures)} memories for PAC ID #{pac_id}."
      )
    end
  ...
  # Not a list or other issue
  defp store_memories(_, _), do: {:ok, []}
  ...
  defp store_single_memory(memory_data, pac_id) do
    ...
    # Ensure pac_id is in the map for creation
    memory_to_create = Map.put(memory_data, :pac_id, pac_id)

    # Changed from store_memory to create_memory
    case MemoryManager.create_memory(memory_to_create) do
      {:ok, stored_memory_struct} ->
        # Generate and store vector embedding
        embedding_text =
          Map.get(
            stored_memory_struct,
            :embedding_text,
            Map.get(memory_data, :embedding_text, "")
          )
        ...
        if String.trim(embedding_text) != "" do
          case VectorSearch.add_embedding(pac_id, stored_memory_struct.id, embedding_text) do
            ...
            {:error, reason} ->
              Logger.warning(
                "MemoryBuilder: Failed to generate/store embedding for memory #{stored_memory_struct.id}: #{inspect(reason)}"
              )
          end
        else
          Logger.warning(
            "MemoryBuilder: Embedding text is empty for memory #{stored_memory_struct.id}. Skipping embedding."
          )
        end
      ...
      {:error, reason} ->
        Logger.error(
          "MemoryBuilder: Failed to store memory for PAC #{pac_id}: #{inspect(reason)}"
        )
  ...
  defp calculate_action_importance(experience) when is_map(experience) do
    ...
    energy_bonus =
      case action_result[:energy_cost] do
  ...
  defp calculate_social_importance(experience, context)
       when is_map(experience) and is_map(context) do
    base = 0.6
    zone_ctx = Map.get(context, :zone_context, %{})

    social_bonus =
      case Map.get(zone_ctx, "pac_count", 0) do
        count when count > 5 -> 0.3
        count when count > 2 -> 0.2
        # Assuming pac_count is total, so > 1 means others
  ...
  defp generate_action_tags(experience) when is_map(experience) do
    ...
    type_tags =
      case action_result[:type] do
        # Assuming :tool_used key
        :tool_success -> ["tool_use", Map.get(action_result, :tool_used, "unknown_tool")]
  ...
  # Simplified: "03h", "14h"
  defp extract_time_of_day(%DateTime{} = datetime),
    do:
